...
{
public class SkipList<T> where T : IComparable, new()
{
    #region "Properties"
    /// <summary>
	...
	#region "Node Class"
	private class Node<TT> : IDisposable where TT : IComparable, new()
	{
		...
		public static bool operator ==(Node<TT> pNode1, Node<TT> pNode2)
		{
			if (pNode1 == null || pNode2 == null)
				return false;
			return pNode1.Value.CompareTo(pNode2.Value) == 0 && pNode2.NextNodes.Length == pNode1.NextNodes.Length;
		}
	...
	private bool Search(T pValue, ref Node<T> pNode)
	{
		Node<T>[] nodesToUpdate = new Node<T>[MAX_LEVEL + 1];

		return Search(pValue, ref pNode, ref nodesToUpdate);
	}
	...
	/// <returns>True if the value is found in the skip list.</returns>
	private bool Search(T pValue, ref Node<T>[] pNodesToUpdate)
	{
		Node<T> retval = new Node<T>();

		return Search(pValue, ref retval, ref pNodesToUpdate);
	}
	...
	/// <param name="pNodesToUpdate">Nodes whose pointers will need updating if the value is inserted.</param>
	/// <returns>True if the value is found in the skip list.</returns>
	private bool Search(T pValue, ref Node<T> pNode, ref Node<T>[] pNodesToUpdate)
	{
        if (pValue == null)
        {
	...
	public void Add(T pValue)
	{
		...
		Search(pValue, ref nodesToUpdate);
	...
	/// <returns>Element stored at th specified index.</returns>
	...
	/// <summary>
	/// Gets and sets the specified value in the skip list
	/// </summary>
	public T this[T pValue] {
		get {
			Node<T> currentNode = new Node<T>();

			//If we find the value we are searching for return it
			if (Search(pValue, ref currentNode)) {
				return currentNode.Value;
			}
		...
		set {
			...
			//If the value exists we need to update it
			if (Search(pValue, ref currentNode, ref nodesToUpdate)) {
				currentNode.Value = value;
			//otherwise insert it into our skip list
			} else {
				Insert(value, nodesToUpdate);
			}
		}
	}
